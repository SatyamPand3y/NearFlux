...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const { isConnected, currentDevice, nearbyDevices, updateRemoteName } = useSocket(deviceName);
  ...
  const selectAllTargetDevices = () => {
    setSelectedTargetDevices([...nearbyDevices]);
  };

  const clearTargetDevices = () => {
  ...
  useEffect(() => {
    ...
    const handleTransferResponse = async (payload: TransferResponsePayload) => {
      ...
      if (currentDevice) {
        ...
        // Hybrid Fallback Timer
        setTimeout(async () => {
          if (!transferStarted && !webrtc.isChannelOpen()) {
            console.warn('[NearFlux] WebRTC direct connection timeout. Switching to Socket.IO fallback stream.');
            transferStarted = true;
            try {
    ...
    const handleWebRTCAnswer = async (payload: WebRTCSignalPayload) => {
      ...
      if (instance) {
        await instance.handleAnswer(payload.signal as RTCSessionDescriptionInit);
      }
    };
    ...
    const handleFileChunk = (payload: FileChunkPayload) => {
      const instance = activeWebRTCInstances.current.get(payload.senderId);
      if (instance) {
        instance.handleIncomingData(payload.data);
      }
    };
    ...
    const handleTransferCancel = () => {
      ...
      // Guard: Only set to cancelled if currently active (ignore if already closed/idle)
      setTransferState((prev) => {
  ...
  // Closes the modal locally without sending socket events if already ended
  const closeTransfer = () => {
    activeWebRTCInstances.current.forEach((instance) => instance.close());
    activeWebRTCInstances.current.clear();
  ...
  // Cancels an active ongoing transfer and notifies peers over socket
